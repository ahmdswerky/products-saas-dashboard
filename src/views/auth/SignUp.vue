<template>
	<div class="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
		<div class="max-w-md w-full space-y-8">
			<div>
				<img class="mx-auto h-16" src="/images/logo.png" alt="Workflow" />
				<h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900">Create a new account</h2>
				<p class="mt-2 text-center text-sm text-gray-600">
					Or
					<RouterLink :to="{ name: 'SignIn' }" class="font-medium text-indigo-600 hover:text-indigo-500">
						Get back to Sign in
					</RouterLink>
				</p>
			</div>
			<Form @submit="register" v-slot="{ errors }" class="mt-8 space-y-2 bg-white p-8 rounded-lg shadow">
				<div class="rounded-md space-y-2">
					<div>
						...
						<div class="text-red-400 text-sm h-5 block mt-1">
							<ErrorMessage v-if="errors.name" name="name" />
						</div>
					</div>
					<div>
				...
				<div>
					<button
						...
						class="relative mt-6s w-full flex justify-center items-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white focus:outline-none"
					>
						<svg
							v-if="loading"
...
import { name, email, password } from '@/schemas/auth';
